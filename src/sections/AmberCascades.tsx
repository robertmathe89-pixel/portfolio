...
function initAmberCascades(canvas: HTMLCanvasElement) {
  ...
  const techSymbols = '()[]{}<>+-*/%=|&^~!01→←↑↓↔↕λΣΩΔΠΨ';
  const codeChars = '01ABCDEF';
  const allChars = techSymbols + codeChars;
  ...
  function render(timestamp: number) {
    ...
    for (const col of columns) {
      ...
      for (let j = 0; j < col.length; j++) {
        ...
        if (j === 0) { r = 0; g = 212; b = 255; }
        else if (j < 3) { r = 0; g = 168; b = 204; }
        else { r = 0; g = 100; b = 130; }
    ...
    waterGrad.addColorStop(0, 'rgba(6, 10, 15, 0.6)');
    waterGrad.addColorStop(1, 'rgba(6, 6, 10, 0.95)');
    ...
    for (const r of ripples) {
      ...
      for (let ring = 0; ring < 3; ring++) {
        ...
        ctx.strokeStyle = `rgba(0, 212, 255, ${alpha * (1 - ring * 0.3)})`;
